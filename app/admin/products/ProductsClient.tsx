...
export default function ProductsClient() {
  ...
  const inputStyle: React.CSSProperties = {
    ...
    background: 'white', fontFamily: 'Manrope, sans-serif', fontSize: '0.875rem',
    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: 'var(--ivory)', fontFamily: 'Manrope, sans-serif' }}>
      <aside style={{ width: '240px', minHeight: '100vh', background: 'var(--balzer-blue-deep)', padding: '2rem 0', flexShrink: 0 }}>
        ...
        <nav>
          {[{ href: '/admin', label: 'Dashboard', icon: '◈' }, { href: '/admin/categories', label: 'Categorie', icon: '◻' }, { href: '/admin/products', label: 'Prodotti', icon: '◯' }, { href: '/admin/qr', label: 'QR Code', icon: '⬡' }].map((l) => (
            <Link key={l.href} href={l.href} style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '0.875rem 1.5rem', textDecoration: 'none', background: l.href === '/admin/products' ? 'rgba(255,255,255,0.08)' : 'transparent', borderLeft: l.href === '/admin/products' ? '2px solid var(--gold)' : '2px solid transparent' }}>
              <span style={{ fontSize: '0.9rem', opacity: 0.7 }}>{l.icon}</span>
              <span style={{ fontFamily: 'Manrope, sans-serif', fontSize: '0.8rem', fontWeight: l.href === '/admin/products' ? 600 : 400, color: l.href === '/admin/products' ? 'white' : 'rgba(255,255,255,0.55)' }}>{l.label}</span>
            </Link>
          ))}
        </nav>
      ...
      <main style={{ flex: 1, padding: '3rem', overflowY: 'auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '2rem', flexWrap: 'wrap', gap: '1rem' }}>
          <div>
            <h1 style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '2.5rem', fontWeight: 400, color: 'var(--balzer-blue)' }}>Prodotti</h1>
        ...
        <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1.5rem', flexWrap: 'wrap' }}>
          <button onClick={() => setFilterCat(null)} style={{ fontFamily: 'Manrope, sans-serif', fontSize: '0.65rem', fontWeight: 600, padding: '0.4rem 0.875rem', background: filterCat === null ? 'var(--balzer-blue)' : 'white', color: filterCat === null ? 'white' : 'var(--text-mid)', border: '1px solid var(--stone)', cursor: 'pointer' }}>
            Tutte
          </button>
          {categories.map((c) => (
            <button key={c.id} onClick={() => setFilterCat(c.id)} style={{ fontFamily: 'Manrope, sans-serif', fontSize: '0.65rem', fontWeight: 600, padding: '0.4rem 0.875rem', background: filterCat === c.id ? 'var(--balzer-blue)' : 'white', color: filterCat === c.id ? 'white' : 'var(--text-mid)', border: '1px solid var(--stone)', cursor: 'pointer' }}>
              {c.name}
            </button>
          ))}
        </div>
              ...
              <thead>
                <tr style={{ background: 'var(--cream)', borderBottom: '1px solid var(--stone)' }}>
                  {['Nome', 'Categoria', 'Prezzo', 'Tags', 'Disponibile', 'Azioni'].map((h) => (
                    <th key={h} style={{ padding: '0.875rem 1rem', textAlign: 'left', fontFamily: 'Manrope, sans-serif', fontSize: '0.62rem', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--text-light)' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {filtered.map((item) => (
                  <tr key={item.id} style={{ borderBottom: '1px solid var(--stone)' }}>
                    <td style={{ padding: '0.875rem 1rem', fontFamily: 'Cormorant Garamond, serif', fontSize: '1rem', color: 'var(--text-dark)' }}>
                      {item.name}
                      {item.description && <div style={{ fontFamily: 'Manrope, sans-serif', fontSize: '0.7rem', color: 'var(--text-light)', marginTop: '0.15rem' }}>{item.description.slice(0, 60)}{item.description.length > 60 ? '…' : ''}</div>}
                    </td>
                    ...
                    <td style={{ padding: '0.875rem 1rem', fontSize: '0.72rem', color: 'var(--gold)' }}>
                    ...
                    <td style={{ padding: '0.875rem 1rem' }}>
                      <div style={{ display: 'flex', gap: '0.75rem' }}>
                        <button onClick={() => openEdit(item)} style={{ fontFamily: 'Manrope, sans-serif', fontSize: '0.72rem', color: 'var(--balzer-blue)', background: 'none', border: 'none', cursor: 'pointer', fontWeight: 600, padding: 0 }}>Modifica</button>
                        <button onClick={() => handleDelete(item.id)} style={{ fontFamily: 'Manrope, sans-serif', fontSize: '0.72rem', color: '#e57373', background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>Elimina</button>
                      </div>
                    </td>
                  </tr>
                ))}
      ...
      {showForm && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem', overflowY: 'auto' }}>
          <div style={{ background: 'white', width: '100%', maxWidth: '560px', padding: '2.5rem' }}>
            <h2 style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '1.8rem', color: 'var(--balzer-blue)', marginBottom: '2rem' }}>
